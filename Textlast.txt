...
class AddEditProductDialog(QDialog):
    def __init__(self, parent=None, product_id=None):
        ...
        self.roll_width_edit.setValidator(QDoubleValidator(0.0, 1000000.0, 2))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.products_table.setStyleSheet("background-color: #BBD9B2;")
        ...
        self.materials_table.setStyleSheet("background-color: #BBD9B2;")
    ...
    def edit_product(self, index=None):
        if isinstance(index, bool):
            index = self.products_table.currentIndex()
        elif index is None:
            index = self.products_table.currentIndex()
        row = index.row()
        if row >= 0:
            ...
            QMessageBox.information(self, "Информация", "Выберите продукт для редактирования.")
    ...
    def edit_material(self, index=None):
        if isinstance(index, bool):
            index = self.materials_table.currentIndex()
        elif index is None:
            index = self.materials_table.currentIndex()
        row = index.row()
        if row >= 0:
